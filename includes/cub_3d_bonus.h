...
int ft_end_column_map(t_file * map_file,
	const int	*h_start_end);
void ft_create_game_map(t_game * game,
	t_file	*map_file, const int *h_start_end,
...
void ft_process_axis_event(t_gamepad * gp,
	struct js_event	event);
...
int ft_try_open_gamepad_at(t_game * game,
...
void ft_set_player_rotation(t_player_actions * act,
	t_gamepad	*gp);
void ft_gamepad_apply_to_player(t_game * game,
	t_gamepad	*gp, int player_idx);
...
void ft_rotate_to_cuadrant(int cuadrant,
...
void ft_draw_tile_edges(t_game * game,
	int	mx, int my, int color);
int	ft_get_tile_color(char tile_char);
t_vector2 ft_world_corner_to_screen(t_game * game,
	double	wx, double wy);
...
t_texture * get_texture_for_wall(t_game * game,
	t_raycast	*ray);
t_texture * get_texture_for_ceiling(t_game * game,
	t_vector2	*pos);
t_texture * get_texture_for_floor(t_game * game,
	t_vector2	*pos);
unsigned int get_fc_color(t_texture * texture,
	t_vector2	*pos);
int ft_calculate_wall_height(t_raycast * ray, int x,
	t_game	*game);
int ft_calc_texture_x(t_raycast * ray,
	t_texture	*texture);
t_vector2 ft_reposition_vector2(t_game * game,
	t_vector2	vec);
...
void ft_draw_pixel_in_img(t_img * img,
	int	x, int y, int color);
void ft_draw_circle(t_game * game, int cx,
	int	cy, int color);
...
void ft_mix_color(unsigned int * color,
...
int ft_find_hottest_reachable_tile(t_game * game,
	t_player	*alien,
t_vector_int	*pos);
int ft_patrol_prepare_step(t_player * alien,
	t_vector2	*target,
...
void ft_debug_print_path(t_game * game, t_vector_int * path,
...
int ft_project_sprite_column(t_game * game, double dx,
	double	dy, t_objet_draw *draw);
...
// ============================================================================
void * ft_create_window(void * mlx, int size_x,
