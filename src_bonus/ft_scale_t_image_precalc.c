...
void ft_scale_t_image_precalc_three(t_texture *tex_origin, t_texture *text_destiny, t_game *game, int player)
{
	int 	dest_x;
	int 	dest_y;
	int		dest_end_y;
	...
	while (dest_y < dest_end_y)
	{
		...
		if (player == 0 || player == 1)
			dest_end_x = text_destiny->width / 2;
		else
		{
			dest_end_x = text_destiny->width;
			dest_x = text_destiny->width / 2;
		}
		while (dest_x < dest_end_x)
		{
			text_destiny->colors_matrix[dest_y][dest_x] = tex_origin->colors_matrix
				[game->precalc.scale_y_table[ori_y]]
				[game->precalc.scale_x_table[ori_x]];
			dest_x++;
			ori_x++;
		}
		dest_y++;
