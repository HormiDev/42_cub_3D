...
/*   Updated: 2026/04/10 01:36:13 by ismherna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (x < 0 || x >= game->width_height[0] || y < 0
		|| y >= game->width_height[1])
		return (1);
 ...
 */
int	ft_is_player_visible(t_game *game, t_player *alien, t_player *target,
		double distance)
{
	...
	angle_diff = ft_normalize_relative_angle(angle_to_target
			- alien->rotation.x);
	if (angle_diff > -90.0 && angle_diff < 90.0)
	{
		ft_raycast(&(t_raycast_input){game, angle_to_target, &ray, distance,
			alien->position});
		if (ray.type == -1)
		{
