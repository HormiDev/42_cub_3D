...
/*   Updated: 2026/04/10 02:42:25 by ismherna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_mix_color(unsigned int *color, unsigned int *mix_color, int percent)
{
	...
	rgba_c[3] = (rgba_c[3] * inv_percent + rgba_m[3] * percent) / 100;
}

/**
 * @brief Mezcla dos colores en un nivel de mezcla dado (0-255).
 *
 * (0 a 255). Si el nivel de mezcla es menor que 0,
	no se realiza ninguna acción. Si es mayor
 ...
 */
void	ft_mix_rgba(unsigned int *color, unsigned int *mix_color, int mix)
{
	unsigned char	*rgba_c;
	unsigned char	*rgba_m;
	int				inv_mix;
