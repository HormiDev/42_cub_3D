...
/*   Updated: 2026/04/10 00:24:10 by ide-dieg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * Dibuja el fondo (techo y suelo) de la pantalla
...
/**
 * @brief get_texture_for_wall - Obtiene la textura correspondiente 
 * @param game: estructura del juego
...
t_texture	*get_texture_for_ceiling(t_game *game, t_vector2 *pos)
{
	...
	y = (int)pos->y;
	if (game->map[y][x] == 'd')
		return (game->arraytextures[6][1]);
	texture_num = (x + y) % game->larraytex[5];
	if (texture_num < 0)
...
t_texture	*get_texture_for_floor(t_game *game, t_vector2 *pos)
{
	...
	y = (int)pos->y;
	if (game->map[y][x] == 'd')
		return (game->arraytextures[6][2]);
	texture_num = (x + y) % game->larraytex[4];
	if (texture_num < 0)
